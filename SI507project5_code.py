...
def write_csv(dict_ls, filename, colnames):
    dict_ls2 = []
    for one_dict in dict_ls:
        csvrow_dict = defaultdict(lambda: "")
        for col in colnames:
            csvrow_dict[col] = one_dict[col]
        dict_ls2.append(csvrow_dict)
    with open(filename, 'w', newline = '', encoding = 'utf-8') as csvfile:
        writer = csv.DictWriter(csvfile, colnames, extrasaction='ignore')
        writer.writeheader()
        for one_dict in dict_ls2:
...
if __name__ == "__main__":
    ...
    # Invoke functions
    tumblr_base_url_1 = "https://api.tumblr.com/v2/blog/{0}/{1}/{2}"
    tumblr_quote_posts_url = tumblr_base_url_1.format('derekg.org', 'posts', 'quote')
    tumblr_quote_posts_params = {}
    tumblr_quote_posts_res = get_data_from_api(tumblr_quote_posts_url,"tumblr",tumblr_quote_posts_params) # Default expire_in_days
    print(type(tumblr_quote_posts_res))
    quote_dict_ls = tumblr_quote_posts_res["response"]["posts"]
    quote_colnames = ["id", "post_url", "date", "summary"]
    write_csv(quote_dict_ls, "quote_posts.csv", quote_colnames)

    tumblr_base_url_2 = "https://api.tumblr.com/v2/{0}"
    tumblr_tagged_posts_url = tumblr_base_url_2.format('tagged')
    tumblr_tagged_posts_params = {'tag': 'gif'}
    tumblr_tagged_posts_res = get_data_from_api(tumblr_tagged_posts_url, "tumblr", tumblr_tagged_posts_params)
    print(type(tumblr_tagged_posts_res))
    tagged_dict_ls = tumblr_tagged_posts_res["response"]
    tagge_colnames = ["blog_name", "post_url", "tags", "summary"]
    write_csv(tagged_dict_ls, "tagged_posts.csv", tagge_colnames)
